module ToBeAble
  class ControllerHelper # :nodoc:
    class << self
      def add_before_action(controller_class, method, *args)
        ...
        controller_class.send(before_action_method, options.slice(:only, :except, :if, :unless)) do |controller|
          controller.class.to_be_able_resource_class.new(controller, resource_name, options.except(:only, :except, :if, :unless)).send(method)
        end
      end
    end
    ...
    def permit_resource
      unless skip?(:permit)
        @controller.permit!(authorization_action, resource_instance || resource_class_with_parent)
      end
    end
    ...
    def skip?(behavior) # This could probably use some refactoring
      options = @controller.class.to_be_able_skipper[behavior][@name]
      if options.nil?
        false
      elsif options == {}
        true
    ...
    def adapter
      ToBeAble::DatabaseAdapter.adapter_class(resource_class)
    end
